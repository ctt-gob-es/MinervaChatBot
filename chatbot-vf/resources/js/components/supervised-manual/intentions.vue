            ...
            <v-container>
                <v-row align="center" justify="center">
                    <v-col cols="auto" class="pa-2"
                        v-if="$can('validate_positive_response_manual') && selectConvertations.length === 0">
                        <v-tooltip text="Tooltip" location="top">
                            <template v-slot:activator="{ props }">
                                <v-btn icon size="small" v-bind="props"
                                    @click="dialog = true, titleModal = 'positiva', descart_id = item.id, thematic = item.subjects_id, intention = item.intention_id, langQuestion = item.language, question = item.question, getIntentions()">
...
const title = 'trainingintentions';
...
const getManualTraining = async () => {
    ...
    axios
        .get('/resourceManualTraining?Q=0&chatbot_id=' + idChatbot.value + '&from=' + from + '&to=' + to)
        .then((response) => {
