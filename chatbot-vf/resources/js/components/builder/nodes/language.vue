<template>
  <div ref="el">
    <div class="title-box"> Selección idioma </div>
    <div class="box text-center">
      <select-language :items="languages" :defaultLanguage="languageSelected" @click-language="changeLanguage"></select-language>
      <v-textarea v-for="(language, i) in languages" :key="i" v-show="languageSelected == dataMessage[i].language" v-model="dataMessage[i].message" label="Mensaje" variant="outlined" class="ma-2" rows="2" hide-details @change="updateData"></v-textarea>
      <div class="box text-center d-flex flex-column">
        <v-btn v-for="(language, i) in languages" :key="i" class="mx-3 my-1" color="success">{{ language.name_setting }}</v-btn>
      </div>
    </div>
  </div>
</template>
...
watch(()=>bus.value.get('Language'), (val) => {
})
...
const validateOutputs = () => {
  if(emptyObject(dataNode.value?.outputs)){
...
const updateData = () => {
  ...
  if(props.languages.length > 0){
    data.info.languages = props.languages.map((item) => ({
      language: item.name_setting
    }))
  }
